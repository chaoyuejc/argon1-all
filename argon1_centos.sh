...
pkglist=(python3-setuptools python3-pip python3-devel gcc libffi libffi-devel i2c-tools)
for curpkg in ${pkglist[@]}; do
	sudo yum install -y $curpkg
...
echo "i2c-dev" | sudo tee -a /etc/modules-load.d/argonone.conf
...
echo '	bus = smbus2.SMBus(1)' >> $shutdownscript
echo 'else:' >> $shutdownscript
echo '	bus = smbus2.SMBus(0)' >> $shutdownscript
...
echo '	bus = smbus2.SMBus(1)' >> $powerbuttonscript
echo 'else:' >> $powerbuttonscript
echo '	bus = smbus2.SMBus(0)' >> $powerbuttonscript
